...
first - get all outages using curl and parse using jq (parser.sh);
second - push all outages to standard input of Go checker (cmd/checker.go);
third - push all outages that should be notified to Python notifier (notifier.py).
